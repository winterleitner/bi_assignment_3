...
for (type in types) {
  outliers <- boxplot(
    car_data[car_data$fuelType == type, ]$mpg, plot=FALSE)$out
  
  if (length(outliers) > 0) {
    car_data <- car_data[-which(car_data$mpg %in% outliers &
                                  car_data$fuelType == type),]
  }
}
```
...
## Identify suitable data mining algorithms
...
## Data Split in train / validation / test set
...
## Model Traning and Parameter Setting
...
relative_error <- abs(cleaned_validate_data$price - gam_pred_eval_2)/cleaned_validate_data$price
nrow(relative_error)
sort(relative_error)[floor(nrow(relative_error)*0.95)]
```
