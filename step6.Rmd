...
Further thought process on this matter in regards of usability in business are present in **ADD REFERENCE HERE!**
...
missing_fact_test <- setdiff(
  unique(test_data$model), unique(train_data$model))
missing_fact_validate <- setdiff(
  unique(validate_data$model), unique(train_data$model))
...
Since the validation set was not utilised until now, neither the model or its parameters are optimised for it. It can be seen as a simulation of new data being applied to it.
```{r}
# Evaluate models
...
## Retraining with train and test data
To further evaluate the model it is now trained utilising all exept the validation data set.
```{r}
...
mean(abs((cleaned_validate_data$price - gam_pred_eval_2)/cleaned_validate_data$price))
```
The mean error does not change significally, which points to good stability of the model. Bringing confidence in its usability with new data collected in future years.
...
```{r}
# relative_error <- cleaned_validate_data$price - gam_pred_eval_2)/cleaned_validate_data$price
relative_error_abs <- abs(cleaned_validate_data$price - gam_pred_eval_2)/cleaned_validate_data$price
error_of_95 <- sort(relative_error_abs)[sort(relative_error_abs)<floor(nrow(relative_error_abs)*0.95)]
ratio_lower_15 <- nrow(relative_error_abs[relative_error_abs < 0.15])/nrow(relative_error_abs)

```

Looking back the initial set goal, it was not possible to achieve the set goal. For 95% of the data a max error **`r ratio_lower_15`** can be ensured, while the absolut mean error for 95% is `r mean(relative_error_abs)`, however this can be positively or negatively which means it can either benefit the customer or the business. The non absolut error is 
# Deployment
