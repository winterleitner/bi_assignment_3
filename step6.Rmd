...
Insert content here.
```{r}
trainsample <- sample(seq_len(nrow(car_data)),size = floor(0.80 * nrow(car_data)))
...
# glm
glm_model <- glm(formula2,data = car_data[trainsample,])
...
# Make predictions
gam_pred <- model %>% predict(cleaned_testing_data)
...
mean(abs((cleaned_testing_data$price - predictions)/cleaned_testing_data$price))
boxplot(cleaned_testing_data$price - predictions)

summary(abs((cleaned_testing_data$price - predictions)/cleaned_testing_data$price))
